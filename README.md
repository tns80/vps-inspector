...
v0.3.2 在 v0.3.1 基础上继续做源码回归修复，重点包括：

- 扫描器自身进程从 `processes` 和“未归属 systemd 的宿主机进程”中排除，避免把 VPS Inspector 自己误当业务工作负载
- `inventory.json` 新增稳定的 `scanner.self_process_excluded` / `excluded_process_count` 元数据，不保存扫描器自身命令行，减少基于关键词检索时的假阳性
- 修复 `::` 与 `0.0.0.0` wildcard 冲突判断不对称的问题；`check` 现在在 `bindv6only=0` 时可双向识别 IPv4/IPv6 dual-stack 冲突
- 命令输出超过上限时 coverage 从 `ok` 改为 `partial`，不再把被截断的采集结果当作完整结果
- 恢复进程 `state`、`threads`、`nspid`、`cwd` 字段，避免压缩重构导致的采集信息回退
- 保留 v0.3.1 的 Tailscale `100.64.0.0/10 -> shared/overlay` 分类、Docker IPv4/IPv6 published-port 归属、非 systemd 宿主机进程和 user-systemd 详细报告
...
```bash
rm -rf /root/vps-snapshot-v032
python3 vps_inspector.py scan -o /root/vps-snapshot-v032
cat /root/vps-snapshot-v032/report.md
```
...
Host processes not attributed to a systemd service
User systemd discovery
Coverage / blind spots
```

Docker 的 `EXPOSE`/声明端口不是宿主机端口占用。工具会尽量把以下三类分开：
...
python3 vps_inspector.py check \
  /root/vps-snapshot-v032/inventory.json \
  my-service.json
```
...
- IPv4/IPv6 wildcard 地址占用关系，包括 dual-stack `::` / `0.0.0.0`
...
- 进程：PID、PPID、用户、state、threads、NSpid、exe、cwd、cmdline、cgroup、netns、systemd service
...
其中 `partial` 也包括命令成功但输出超过采集上限而被截断的情况。
